...
export default async function UserProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const user = await getUserWithOffers(id)
    ...
    <AuthLayout>
      <main className="p-6 max-w-md mx-auto">
        ...
        {isOwnProfile && (
          <Link
            href="/profile"
            className="block w-full bg-tan text-black border border-black rounded-sm py-3 text-sm hover:bg-black hover:text-tan transition-colors text-center"
          >
            Edit Profile
          </Link>
        )}
